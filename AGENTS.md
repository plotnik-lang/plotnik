...
A subtler outside-the-boundary failure is a *native-stack overflow* or *unbounded heap* on untrusted input — an abort, not a clean error. So: a resource-limit or stack-safety change must enumerate **every** native-recursive path and **every** unbounded heap the pipeline owns — execution, output formatting, serialization, verification, CLI tree walks — and show each is iterative or bounded, before claiming the class is closed. A property test (`fuzz_no_panic`) is a probabilistic safety net for the thesis — arbitrary query × source ⇒ always a `Result`, never a panic or abort — backstopping the deterministic depth proof in `stack_safety`, not standing in for the enumeration above.
