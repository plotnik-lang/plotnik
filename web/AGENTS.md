...
| Panes         | `tree-view.tsx`, `output-panel.tsx`, `code-editor.tsx`                      | Presentation over session state; no async, no client                                                                        |
| Interactions  | `spotlight.ts` + adapters `cm-spotlight.ts`, `tree-dom.ts`; `tree-index.ts` | `spotlight.ts` is pane-agnostic; adapters translate pane geometry into it; `tree-index.ts` stays DOM-free                   |
