...
#[test]
fn memory_limit_text_is_human_sized() {
    let msg = render_runtime_error(&RuntimeError::MemoryLimitExceeded(64 << 20), false);
    assert!(msg.contains("memory limit exceeded"));
    assert!(msg.contains("[E-limit-memory]"));
    assert!(msg.contains("64.0 MiB"));
...
fn json_form_is_compact_machine_readable() {
    ...
    let mem = render_runtime_error(&RuntimeError::MemoryLimitExceeded(1024), true);
    assert_eq!(
        mem,
        r#"{"error":"limit-exceeded","code":"E-limit-memory","max_memory":1024}"#
    );
}
