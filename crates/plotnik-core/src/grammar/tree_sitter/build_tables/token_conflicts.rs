...
pub struct TokenConflictMap<'a> {
    ...
    pub(crate) conflict_or_prefix_bits: Vec<BitBlock>,
    /// Bit `j` is set iff [`does_overlap(i, j)`](Self::does_overlap) || [`does_overlap(j, i)`](Self::does_overlap).
    pub(crate) overlap_either_bits: Vec<BitBlock>,
...
impl<'a> TokenConflictMap<'a> {
    ...
    pub fn new(grammar: &'a LexicalGrammar, following_tokens: Vec<TokenSet>) -> Self {
        ...
        let mut conflict_or_prefix_bits = vec![BitBlock::default(); n * row_words];
        let mut overlap_either_bits = vec![BitBlock::default(); n * row_words];
        for i in 0..n {
            let row_base = i * row_words;
            for j in 0..n {
                let entry_ij = status_matrix[matrix_index(n, i, j)];
                if entry_ij.intersects(conflict_mask) {
                    conflict_or_prefix_bits[row_base + j / bits_per_word] |=
                        BitBlock::from(1u8) << (j % bits_per_word);
                }
                let entry_ji = status_matrix[matrix_index(n, j, i)];
                if entry_ij.intersects(overlap_mask) || entry_ji.intersects(overlap_mask) {
                    overlap_either_bits[row_base + j / bits_per_word] |=
                        BitBlock::from(1u8) << (j % bits_per_word);
                }
            }
        }
    ...
    pub fn does_match_valid_continuation(&self, i: usize, j: usize) -> bool {
        self.status_matrix[matrix_index(self.n, i, j)]
            .contains(TokenConflictStatus::DOES_MATCH_VALID_CONT)
    }

    #[must_use]
    pub fn does_match_separators(&self, i: usize, j: usize) -> bool {
        self.status_matrix[matrix_index(self.n, i, j)]
            .contains(TokenConflictStatus::DOES_MATCH_SEPARATORS)
    }

    #[must_use]
    pub fn starting_chars(&self, index: usize) -> &CharacterSet {
        &self.starting_chars_by_index[index]
    }
