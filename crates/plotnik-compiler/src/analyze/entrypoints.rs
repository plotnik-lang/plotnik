...
/// from `TypeContext::iter_def_types()`. The AST is the source of truth for the
...
pub fn validate_entrypoints(
    ...
    let typed: HashSet<Symbol> = type_context
        .iter_def_types()
        .map(|(def_id, _)| type_context.def_name_sym(def_id))
        .collect();
