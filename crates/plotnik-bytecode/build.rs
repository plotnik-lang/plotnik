...
// package (a dev-dependency, via `plotnik-compiler`) and expose its path so the
// forged-module tests can build a real grammar to emit fixtures against.
fn main() {
    ...
    println!("cargo::rustc-env=PLOTNIK_BYTECODE_JAVASCRIPT_GRAMMAR_JSON={grammar_path}");
