...
impl<T> Sink<T> {
    ...
    pub(crate) fn render(&self, colors: Colors) -> String {
        let mut out = String::with_capacity(
            self.text.len() + self.styles.len().saturating_mul(colors.reset.len()),
        );
        let mut cursor = 0;
        for event in &self.styles {
            out.push_str(
                self.text
                    .get(cursor..event.offset)
                    .expect("style offsets are emitted on UTF-8 boundaries in order"),
            );
            out.push_str(match event.change {
