...
fn invalid_containment_propagates_through_recursive_cycle() {
    ...
    let declaration = TypeDeclId::from_raw(0);
    let types = vec![
        None,
        Some(TypeShape::Node),
        Some(TypeShape::Text),
    ...
    let declarations = BTreeMap::from([(declaration, recursive)]);

    let contained = compute_invalid_containment(&types, &declarations, &HashSet::from([invalid]));
